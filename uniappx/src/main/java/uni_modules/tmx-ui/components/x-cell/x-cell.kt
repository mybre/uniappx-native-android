...
open class GenUniModulesTmxUiComponentsXCellXCell : VueComponent {
    ...
    override fun `$render`(): Any? {
        ...
        return createElementVNode("view", mergeProps(_ctx.`$attrs`, utsMapOf("onClick" to _ctx.clickLisent, "hover-start-time" to if (_ctx._isLinksHover) {
            50;
        } else {
            0;
        }
        ...
        , "class" to utsArrayOf(
            "xCell",
            utsArrayOf(
                if (_ctx._allAttr.card == true) {
                    "cellCard";
                } else {
                    "cardInset";
                }
            )
        ), "style" to utsMapOf("backgroundColor" to _ctx._color, "borderRadius" to if (_ctx._allAttr.card == true) {
            _ctx._cardRadius;
        } else {
            "0px";
        }
        , "minHeight" to _ctx._minHeight, "padding" to _ctx._padding, "borderBottom" to if (_ctx._allAttr.bottom && !_ctx._allAttr.card && !_ctx.bottomBorderInsert) {
                    ...
                    renderSlot(_ctx.`$slots`, "label", utsMapOf("label" to _ctx._allAttr.label), fun(): UTSArray<Any> {
                        return utsArrayOf(
                            if (_ctx._allAttr.label != "") {
                                createElementVNode("text", utsMapOf("key" to 0, "style" to normalizeStyle(utsMapOf("color" to _ctx._allAttr.labelColor, "fontSize" to _ctx._rightLableSize)), "class" to "rightLabel"), toDisplayString(_ctx._allAttr.label), 5);
                            } else {
                                createCommentVNode("v-if", true);
                            }
                        );
    ...
    open lateinit var clickLisent: () -> Unit;
    companion object {
