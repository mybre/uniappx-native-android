...
open class GenUniModulesTmxUiComponentsXImageXImage : VueComponent {
    ...
    override fun `$initMethods`() {
        ...
        this.prevImage = fun() {
            this.`$emit`("click");
            if (this.preview) {
                uni_previewImage(PreviewImageOptions(current = this.src, urls = utsArrayOf(
                    this.src
                )));
            }
        }
        ;
        this.imgLoad2 = fun(evt: ImageLoadEvent) {
    ...
    open lateinit var setIsVisibledShow: () -> Unit;
    open lateinit var prevImage: () -> Unit;
    open lateinit var imgLoad2: (evt: ImageLoadEvent) -> Unit;
    open lateinit var imgLoad: () -> Unit;
    open lateinit var imgError: () -> Unit;
    open lateinit var mStart: (evt: UniTouchEvent) -> Unit;
    open lateinit var mEnd: (evt: UniTouchEvent) -> Unit;
    open lateinit var getNodes: () -> Unit;
    open lateinit var resize: () -> Unit;
    companion object {
